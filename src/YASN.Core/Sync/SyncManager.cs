...
namespace YASN.Sync
{
    ...
    public class SyncManager : IDisposable
    {
        ...
        private readonly ISyncAppBridge? _appBridge;
        ...
        /// </summary>
        public SyncManager(ISyncAppBridge? appBridge = null)
        {
        ...
        private async Task<SyncResult> SyncAsync()
        {
            ...
            try
            {
                ...
                if (shouldReloadNotes)
                {
                    if (_appBridge != null)
                    {
                        await _appBridge.ReloadNotesAsync();
                    }
                }
                ...
                if (shouldRefreshPreviewStyles)
                {
                    if (_appBridge != null)
                    {
                        await _appBridge.RefreshPreviewStylesAsync();
                    }
                }
        ...
        private async Task<ConflictResolutionAction> PromptConflictAsync(string fileName, DateTime localTime, DateTime remoteTime)
        {
            if (_appBridge == null)
            {
                return ConflictResolutionAction.KeepLocal;
            }

            return await _appBridge.ResolveConflictAsync(fileName, localTime, remoteTime);
        }
